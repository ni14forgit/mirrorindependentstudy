...
def skin_retrieve():
    originalFile_Dir = nonML + "data/demo_images"      
    croppedFaces_Dir = nonML +"data/demo_images_patches" 
    Eye_Cascade_Path = nonML +"models/haarcascade_eye.xml"
    PREDICTOR_PATH = nonML +"models/shape_predictor_68_face_landmarks.dat"
    ...
    def draw_convex_hull(im, points, color):
        ...
        # print("points end")
        # print(type(points))
        #points = cv2.convexHull(cv2.UMat(points))
        points = cv2.convexHull(np.float32(points))
        cv2.fillConvexPoly(im, np.array(points, 'int32'), color=color)
    ...
    for imagefile in imageFiles[start_index:end_index]:
        ...
        if face_detected: # if face is detected by landmark model
            ...
            new_face_y_min = max(0, face_y_min - forehead_height) # new_face_y_min is the top edge of the forehead. 
             ...
             ## INSERT: 
            face_region = cv2.cvtColor(img[new_face_y_min:face_y_max, face_x_min:face_x_max, :],cv2.COLOR_BGR2RGB)
            imageio.imwrite("nonML/data/demo_images_patches/test.jpg", face_region)
